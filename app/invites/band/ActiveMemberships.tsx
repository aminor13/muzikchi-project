...
export default function ActiveMemberships({ memberships, userId }: ActiveMembershipsProps) {
  ...
  const handleLeave = async (membershipId: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'خطا در خروج از گروه');
      }
      ...
      alert(error.message || 'خطا در خروج از گروه');
    } finally {
  ...
  if (memberships.length === 0) {
    return (
      <div className="bg-gray-900 border border-gray-100 rounded-lg shadow-lg p-6 text-center">
        <p className="text-gray-300">در حال حاضر عضو هیچ گروهی نیستید</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <ConfirmationModal
        ...
        title="تایید خروج از گروه"
        message={`آیا از خروج از گروه "${modalConfig.bandName}" اطمینان دارید؟`}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              {membership.band.avatar_url && (
                <div className="relative w-16 h-16 rounded-full overflow-hidden">
                  <Image
                    ...
                    alt={membership.band.name || membership.band.display_name}
                    fill
              ...
              <div>
                ...
                >
                  {membership.band.name || membership.band.display_name}
                </Link>
                {membership.band.description && (
                  <p className="text-white mt-1 line-clamp-2">{membership.band.description}</p>
                )}
            ...
            <button
              onClick={() => setModalConfig({
                ...
                bandName: membership.band.name || membership.band.display_name
              })}
